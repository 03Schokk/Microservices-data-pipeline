...
if ($report.lectures.Count -eq 0) {
    ...
    $lines += "-" * 165
    $lines += ("{0,-25} {1,-30} {2,-10} {3,-35} {4,-10} {5,-40}" -f "Курс", "Лекция", "Тип", "Требования к оборуд.", "Студ.", "Описание курса")
    $lines += "-" * 165
    ...
    foreach ($lec in $report.lectures) {
        ...
        $equip  = if ($lec.computer_type.Length -gt 35) { $lec.computer_type.Substring(0, 32) + "..." } else { $lec.computer_type }
        $count  = $lec.student_count
        $desc   = if ($lec.course_description.Length -gt 40) { $lec.course_description.Substring(0, 37) + "..." } else { $lec.course_description }
        $lines += ("{0,-25} {1,-30} {2,-10} {3,-35} {4,-10} {5,-40}" -f $course, $title, $type, $equip, $count, $desc)
    }
